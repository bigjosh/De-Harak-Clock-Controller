...
#define ROWS_PER_PIN (((FRAME_SIZE_Y-1) / PIN_COUNT)+1)

#define OPC_BYTECOUNT (FRAME_SIZE_X*(PIN_COUNT*ROWS_PER_PIN)*3)       // 3 bytes per pixel for RGB
    ...
    for( int pin = 0; pin <PIN_COUNT ; pin++ ) {                // OPC packet has each string snet sequentially
        ...
        for( int row=0; row < ROWS_PER_PIN ; row++ ) {
           ...
           // printf("p,r,y=%d,%d,%d\r\n",pin,row,y);
            ...
            if (y>=FRAME_SIZE_Y) {      // Of the screen?
                                
                for( int i=0;i<FRAME_SIZE_X * 3;i++) {
